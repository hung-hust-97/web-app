...
const AllMarketInfTable = ({data, activeCurrency}) => {
    ...
    let head = (
        <div className="row text-gray px-2 py-2" style={{backgroundColor:"var(--tableHeader)"}}>
            ...
            <span className="width-15 flex  jc-start ai-center">{t("MarketInfo.lastPrice")}</span>
            <span className="width-12 flex jc-start ai-center">{t("MarketInfo.priceChange")}</span>
            <span className="width-15 flex jc-start ai-center">{t("MarketInfo.lowPrice")}</span>
            <span className="width-15 flex jc-start ai-center">{t("MarketInfo.highPrice")}</span>
            ...
            {/* <span className="width-15 flex jc-start ai-center">{t("MarketInfo.chart")}</span>{/* */}
            ...
            <span className="width-8 flex jc-center ai-center">{t("MarketInfo.trade")}</span>*/} 
             <span className="width-16 flex jc-start ai-center">{t("MarketInfo.tools")}</span>
        </div>
    );
            ...
            {data.map((tr, index) => {
                return (
                    <div className={`${classes.row} row rounded-5 border-bottom px-2 py-2`} key={index}>
                        ...
                        <span className={`width-15 flex jc-start ai-center ${tr?.priceChangePercent > 0 ? "text-green" : "text-red"}`}>{new BN(tr.lastPrice).toFormat()} <span className={`fs-0-7 mr-05`}>{t("currency." + tr?.quote)}</span></span>


                        <span className={`width-12 flex ${i18n.language !== "fa" ? 'jc-start' : 'jc-end'} ai-center ${tr?.priceChangePercent > 0 ? "text-green" : "text-red"} direction-ltr`}>{new BN(tr?.priceChangePercent).toFormat(2)} %</span>
                        <span className="width-15 flex jc-start ai-center">{new BN(tr?.lowPrice).toFormat()}</span>

                        <span className={`width-15 flex jc-start ai-center`}>{new BN(tr?.highPrice).toFormat()}</span>
...
{/* 
                        <span className="width-15 flex jc-start ai-center position-relative">
                            <img
                        ...
                        <span className="width-8 flex jc-start ai-center">
                            <Button
