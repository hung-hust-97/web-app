...
const Landing = () => {
  return (
    <div
      className={`${classes.container} move-image`}
      style={{backgroundImage: `url("${images.spaceStar}")`}}>
      <div
        className={`row jc-between ai-center width-90 m-auto`}
        style={{height: "55vh"}}>
        <div
          className={`width-30 column jc-between ai-start`}
          style={{height: "55vh"}}>
          <MarketTitle />
          <GeneralInfo />
        </div>
        <div className={`width-40  jc-end`}>
          <Spinner />
        </div>
      </div>
      <div
        className={`flex jc-center mb-5`}
        style={{height: "", backgroundColor: "var(--mainContent)"}}>
        <MarketView />
      </div>
      <div
        className={`flex jc-center mb-5`}
